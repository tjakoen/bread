// portfolio/e2e/editor-tabs.e2e.ts — THE EDITOR v2: the explorer file tree + the open-pages tab
...
test.describe("THE EDITOR v2 — explorer + open tabs", () => {
  test("folders ship collapsed; the current page's ancestors unfold and the file is marked", async ({ page }) => {
    ...
    // NB exact summary text — :has-text would also match ancestors ("batch/index.html" in portfolio/).
  ...
  test("zero-JS fallback: the pinned tab renders and the tree still navigates", async ({ browser }) => {
    ...
    const portfolioDir = page.locator('.file-tree > .file-tree__dir:has(> summary:text-is("portfolio/"))');
